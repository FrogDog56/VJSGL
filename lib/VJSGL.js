...
/*
    @TODO
    Key Input - let user press two keys at once
...
class Background extends Item {
    ...
    draw() {
        let ctx = getCtx();
        ctx.beginPath();
        ctx.drawImage(this.image, 0, 0, getWidth(), getHeight());
        ctx.closePath();
    }
}
...
class ShapeItem extends Item {
    ...
    // standard getters and setters dont work here
    getX() {
        return this.x;
    }
...
class Rectangle extends ShapeItem {
...
class Sprite extends ShapeItem {
    constructor(image, x, y ,width, height, speed) {
        super();
        this.image = image;
        this.x = x;
...
const yellow = new Colour("#FFFF00");
